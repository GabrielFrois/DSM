...
--2 Liste os títulos (tbl_titulo) e seus status (tbl_livros), 
--incluindo os que não têm status definido. (retorna 6 linhas)
SELECT tbl_titulo.titulo, tbl_livros.status FROM tbl_titulo
LEFT JOIN tbl_livros ON tbl_titulo.codigo_titulo = tbl_livros.codigo_titulo;

--3 Liste os títulos (tbl_titulo.titulo) e suas descrições (tbl_titulo.decricao) dos 
-- livros alugados (tbl_livros.status) (retorna 4 linhas)
SELECT tbl_titulo.titulo, tbl_titulo.descricao, tbl_livros.status 
FROM tbl_titulo
INNER JOIN tbl_livros ON tbl_titulo.codigo_titulo = tbl_livros.codigo_titulo
WHERE tbl_livros.status = 'ALUGADO';

--4 Liste os nomes dos clientes que não têm livros alugados (retorna 1 linha)
--tbl_cliente tbl_livros tbl_emprestimo
SELECT tbl_cliente.nome FROM tbl_cliente
INNER JOIN tbl_emprestimo ON tbl_cliente.codigo_cliente = tbl_emprestimo.codigo_cliente
INNER JOIN tbl_livros ON tbl_emprestimo.codigo_livro = tbl_livros.codigo_titulo
WHERE tbl_livros.status != 'ALUGADO';
